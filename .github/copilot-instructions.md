...
| `@rhdh-architect` | `@rhdh-architect Design a custom plugin` | RHDH/Backstage plugin architecture, frontend wiring, component specs, ADRs |
...
**Tool Hierarchy (MCP-First Policy):** Azure Copilot + Azure MCP (`azure-mcp/*`, `com.microsoft/azure/*`) are the **PRIMARY** methods for Azure operations. CLI scripts in `scripts/` serve as **guardrails** for validation only. Agents MUST prefer MCP tools over direct CLI when available.
...
| `config/region-availability.yaml` | Azure region matrix with Tier 1/2 support, AI model availability per region, quota requirements per deployment mode, and deployment patterns. **Agents MUST consult this file before recommending or provisioning any Azure service in a region.** Official reference: https://azure.microsoft.com/en-us/explore/global-infrastructure/products-by-region/table |
